...
if uploaded_file is not None:
    ...
    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
        ...
        for col_num, value in enumerate(registration_df.columns.values):
            worksheet.write(2, col_num, value, header_format)  # 3행(0-based index 2)에 헤더 작성
            
        # 데이터 서식 적용
        for row_num in range(len(registration_df)):
            worksheet.set_row(row_num+3, 22)  # 4행부터 시작 (0-based index 3)
            for col_num, value in enumerate(registration_df.iloc[row_num]):
                worksheet.write(row_num+3, col_num, value, cell_format)
        
        # 모든 페이지에 헤더 반복 설정 (4행 = 0-based index 3)
        worksheet.repeat_rows(3)  # 4행(헤더)을 모든 페이지에 반복
    ...
    st.success("표 테두리가 적용되었으며, 인쇄 시 모든 페이지에 항목명이 반복됩니다.")
else:
    st.info("CSV 파일을 업로드하면 편집 가능한 엑셀 파일을 다운로드할 수 있습니다.")
